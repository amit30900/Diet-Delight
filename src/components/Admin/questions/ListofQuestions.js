...
import { makeStyles, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Button, Dialog, DialogTitle, DialogContent, Snackbar, Select, MenuItem } from '@material-ui/core'

import MuiAlert from '@material-ui/lab/Alert'

import { useDispatch, useSelector } from 'react-redux'
import { setQuestion, selectQuestion, resetQuestion } from '../../../features/adminSlice'

import { Main, HContainer, Con, Input, Title, Set, Mini, Info, Container } from './QuestionElements'
...
const ListofQuestions = () => {
    ...
    useEffect(() => {
        axios.get(`questions?pageSize=${page}&search=${search}&sortBy=${sort}&sortOrder=${order}`, {
            headers: {
    ...
    return (
        <>
                ...
                <DialogContent>
                    <Mini>
                        <Button variant="contained" color="secondary"
                            onClick={() => {
                                axios.delete(`questions/${question.id}`).then(
                                    (res) => {
                                        setIsSuccess(true)
                                        setIsDelete(false)
                                        handleShow()
                                ...
                                <Container>
                                    ...
                                    <Mini style={{ marginTop: "10px" }}>
                                        ...
                                        <Button variant="contained"
                                            color="primary" onClick={CloseUpdate}>Close</Button>
                                    </Mini>
                                </Container>
                            </>
                        )}
            ...
            {loading ? (<CustomSkeleton />) : (
                <>
                    <Main>
                        <h3 style={{ textAlign: "left", marginLeft: "50px", marginBottom: "20px" }}>GET Questions</h3>
                        <HContainer>
                            <Con>
